...
const chatSlice = createSlice({
    ...
    reducers: {
        ...
        removeMessage(state, action) {
            ...
            const arr = state.messagesMap[chatKey]?.messages;
            if (!arr) return;
            state.messagesMap[chatKey].messages = arr.filter((m) => m.id !== id);
        },
        ...
        updateMessage(state, action) {
            ...
            const msgIndex = messages.messages.findIndex(m => String(m.id) === String(messageId));
        ...
        updatePollVote(state, action) {
            const {chatKey, pollId, optionId, userId, action: voteAction} = action.payload;
            ...
            const normalizedUserId = typeof userId === "string" && userId.startsWith("user:")
                ? userId
                : `user:${userId}`;
            ...
            if (voteAction === "add") {
                ...
                targetOption.voters = targetOption.voters.filter(v => v !== normalizedUserId);
            }
            ...
            newPoll.totalVotes = newPoll.options.reduce((sum, opt) => sum + (opt.votes || 0), 0);
