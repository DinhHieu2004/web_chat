...
export default function MessageReactionBar({
  ...
  const isSender = currentUser === message?.from;
  ...
  return (
    !isSender && (
      ...
      >
        ...
        {open && (
          <div
            className="absolute bottom-full left-1/2 -translate-x-1/2 bg-white border border-gray-200 shadow-lg rounded-full px-2 py-1 flex gap-1 z-50"
          >
            {REACTION_EMOJIS.map((unified) => {
              ...
              return (
                <button
                  key={unified}
                  type="button"
                  className={`text-xl px-1 transition-transform duration-150 hover:scale-125 ${
                    active ? "scale-125" : ""
                  }`}
                  onClick={() => {
                    if (currentUserKey) {
