  ...
  dark:bg-white/10 dark:text-gray-200 dark:border-white/15"
    ...
    const renderContent = () => {
        ...
        if (finalType === "poll") {
            ...
            const currentUserId =
                typeof currentUser === "string" && currentUser.startsWith("user:")
                    ? currentUser
                    : `user:${currentUser}`;
            ...
            return (
                <div className="flex flex-col gap-3 min-w-[280px]">
                    <div className="font-semibold text-base">{poll.question}</div>

                    <div className="flex flex-col gap-2">
                        {poll.options.map((opt) => {
                            const currentVotersCount = opt.voters?.length || 0;
                            const totalVotersCount = poll.options.reduce((acc, curr) => acc + (curr.voters?.length || 0), 0);

                            const percentage = totalVotersCount > 0
                                ? Math.round((currentVotersCount / totalVotersCount) * 100)
                                : 0;
                            ...
                            return (
                                ...
                                >
                                    {/* Progress bar */}
                                    <div
                                        className={`absolute left-0 top-0 h-full transition-all ${isVotedByMe ? "bg-white/20" : "bg-purple-200"
                                            }`}
                                    ...
                                    <div className="relative flex items-center gap-2 ml-3">
                                        ...
                                        <span className="text-xs opacity-75">({currentVotersCount})</span>
                                    </div>
                                </button>
                            );
                        })}
                    ...
                    {/* Footer */}
                    <div className="text-xs text-gray-500 flex items-center justify-between pt-1 border-t border-gray-200">
                        <span>
                            {totalVotes} {totalVotes === 1 ? "lượt bình chọn" : "lượt bình chọn"}
                        </span>
                        {hasVoted && (
                            <span className="text-purple-600 font-medium">✓ Đã bình chọn</span>
                        )}
                    </div>
                </div>
            );
            ...
            return (
                <div className="min-w-[220px] py-1">
                    <div className="flex items-center gap-3 mb-3">
                        <div
                            className={`p-2.5 rounded-full ${isMe
                        ...
                        <div className="flex-1">
                            <div
                                className={`font-bold text-sm ${!isMe && isMissed ? "text-red-500" : ""
                                    }`}
                            >
                                {callData?.text ||
                            ...
                            <div
                                className={`text-[11px] ${isMe ? "text-purple-100" : "text-gray-500"
                                    }`}
                            >
                                {isMissed
                                    ? "Nhấn để gọi lại"
        ...
        return (
            <div className="flex flex-col gap-2">
                ...
                {finalType !== "emoji" &&
                    ...
                    safeText.trim() !== "" && (
                        <p
                            className={`text-sm wrap-break-words whitespace-pre-wrap ${finalUrl ? "mt-2" : ""
                                }`}
                        >
    ...
    return (
      ...
      opacity-0 group-hover:opacity-100 text-gray-400 hover:text-gray-700 transition p-2`}
                    ...
                    {openOpt && (
                        <div
                            className={`absolute top-6 ${isMe ? "right-full mr-2" : "left-full ml-2"
                                } w-52
