...
export default function useChatLogic({ activeChat, setActiveChat, currentUser }) {
  ...
  const senderOptions = useMemo(() => buildSenderOptions(messages || []), [messages]);
  ...
  const handleConfirmForward = ({ selectedMap, note }) => {
    ...
    keys.forEach((k) => {
      ...
      // optimistic message into that target chat key
  ...
  useEffect(() => {
    ...
    const onIncoming = (payload) => {
      ...
      const parsed = tryParseCustomPayload(typeof mes === "string" ? mes : mes?.mes);
