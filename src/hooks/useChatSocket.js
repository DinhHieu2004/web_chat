...
export function useChatSocket(currentUser, callLogic) {
  ...
  useEffect(() => {
    ...
    const resolveActionTime = (obj) => {
      if (typeof obj?.actionTime === "number" && obj.actionTime)
    ...
    const refreshHistoryByChatKey = (ck, page = 1) => {
      if (!ck) return;

      const [prefix, name] = String(ck).split(":");
      if (!name) return;

      const chat = {
        type: prefix === "group" ? "room" : "people",
        name,
      };
    ...
    const onIncoming = (payload) => {
      ...
      const meKey = normalizeUserKey(currentUser);

      if (parsed?.customType === "delete_for_me") {
        const target = normalizeUserKey(parsed.user);
        if (chatKey && parsed.messageId && target && meKey === target) {
          dispatch(removeMessage({ chatKey, id: parsed.messageId }));
        }
        return;
      }

      if (parsed?.customType === "restore_for_me") {
        const target = normalizeUserKey(parsed.user);
        if (chatKey && parsed.messageId && target && meKey === target) {
          refreshHistoryByChatKey(chatKey, 1);
        }
        return;
      }

      if (parsed?.customType === "recall") {
        if (chatKey && parsed.messageId) {
          dispatch(recallMessage({ chatKey, id: parsed.messageId }));
        }
        return;
      }
      ...
      if (isPoll) {
        displayText =
          parsed?.poll?.question || parsed?.question || "Biểu quyết";
      } else if (isCallLog) {
    ...
    const onHistory = (payload) => {
      ...
      const mapHistoryMessage = (m) => {
        ...
        if (
          ...
          parsed?.customType === "delete_for_me" ||
          parsed?.customType === "restore_for_me" ||
          parsed?.customType === "recall"
        ) {
          return null;
        }
      ...
      if (event === "GET_PEOPLE_CHAT_MES" && Array.isArray(data)) {
        ...
        raw.forEach((m) => {
          ...
          if (parsed?.customType === "recall") {
            recallEvents.push({ messageId: parsed.messageId });
            return;
          }
        ...
        deleteOps.forEach((op) => {
          const target = normalizeUserKey(op.user);
          if (!meKey || !target || target !== meKey) return;

          const mid = String(op.messageId);
          if (op.kind === "delete_for_me") deletedSet.add(mid);
          if (op.kind === "restore_for_me") deletedSet.delete(mid);
        });
