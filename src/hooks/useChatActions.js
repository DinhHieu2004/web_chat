...
export function useChatActions({
        ...
        dispatch(
            addMessage({
                ...
                message: {
                    ...
                    sender: "user", 
                    optimistic: true,
                    meta: replyMsg ? { reply: { preview: getMessagePreview(replyMsg) } } : null,
                },
            })
        );
    ...
    return {
        handleSend: (textValue) => {
            ...
            const mes = attachReplyMeta(hasEmoji(text) ? buildEmojiMessage(text) : text);
        ...
        handleFileUpload: async (file, currentInput) => {
            ...
            try {
                ...
                const payload = attachReplyMeta(
                    JSON.stringify({ customType: type, url, text: currentInput, fileName: file.name })
                );
                commonEmit(payload, { text: currentInput, type, url, fileName: file.name });
            } finally {
        ...
        handleSendRichText: (editorRef) => {
            ...
            if (!rich) return;
            const payload = attachReplyMeta(
                JSON.stringify({ customType: "richText", text: rich.text, blocks: rich.blocks })
            );
            commonEmit(payload, {
                type: "richText",
                text: rich.text,
