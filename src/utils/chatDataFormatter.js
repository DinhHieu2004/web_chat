...
const makePreviewText = (origin) => {
  ...
  if (origin?.type === "emoji") return t;

  if (MEDIA_TYPES.has(origin?.type)) return t;
...
export const buildPollVote = (pollId, optionId, userId) => {
  if (!pollId || !optionId || !userId) return null;
...
export const updatePollWithVote = (poll, optionId, userId) => {
  if (!poll || !optionId || !userId) return poll;
  ...
  const targetOption = updatedPoll.options.find((opt) => opt.id === optionId);
  if (targetOption) {
...
export const removePollVote = (poll, optionId, userId) => {
  if (!poll || !optionId || !userId) return poll;
